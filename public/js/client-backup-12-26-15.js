...
$(function() {
  ...
  if(touch.clientX < window.innerWidth/3) {
	...
	function onKeyDown(event) {
		...
		switch (event.keyCode) {
			case inputLayout.jump: //space
				...
				input.action.moveBackwardpace = true;
	...
	function onKeyUp(event) {
		...
		switch (event.keyCode) {
			case inputLayout.jump: //space
				...
				input.action.moveBackwardpace = false;
	...
	socket.on('disconnect', function() {
		//swal("Server disconnected / reset!", "", "error");
		//location.reload();
		swal({
		...
		for (var i = 0; i < vpd.length; i++) {
			...
			if (vpd[i].type == "player") {
				if (vpd[i].username == world1.game.player.username) {
					...
					//console.log("Your health: " + vpd[i].health);
	...
	function world() {
		this.createCanvas = function(location, id, width, height) {
			...
			this.t.listener = new THREE.AudioListener();
 +		this.t.camera.add(this.t.listener);
			this.t.raycaster = new THREE.Raycaster();
			this.t.HUD = new THREE.Scene();
			//this.t.HUDCamera = new THREE.OrthographicCamera(-this.width/2, this.width/2, this.height/2, -this.height/2, 1, 10);
			this.t.HUDCamera = new THREE.OrthographicCamera(-this.width/2, this.width/2, this.height/2, -this.height/2, 1, 1000);
			this.t.HUDCamera.up.set(0, 0, 1);
			this.t.HUDCamera.position.set(0, 0, 10);
			this.t.HUDRaycaster = new THREE.Raycaster();
	...
	console.log(fxPos);
	console.log(fyPos);
	world1.t.HUD.add(healthBarMesh);*/

	var healthBar = createHealthBar();
	world1.t.HUD.add(healthBar.mesh);
	
	var XPBar = createXPBar();
	...
	window.addEventListener('resize', function() {
		...
		world1.t.HUDCamera.aspect = window.innerWidth / window.innerHeight;
		world1.t.HUDCamera.updateProjectionMatrix();
	...
	function gameLoop(world) {
		if (world.game.connected) {
			...
			if (input.action.moveBackwardpace && temp.isJumping === false) {
				temp.isJumping = true;
			...
			/*if (input.action.moveForward && input.action.moveLeft) {
				wasKeyPressed = true;
				dirOffset = 3 * Math.PI / 4;
				//dirOffset = findNearestCoterminalAngle(pry, 3*Math.PI/4);
			} else if (input.action.moveForward && input.action.moveRight) {
				wasKeyPressed = true;
				dirOffset = Math.PI / 4;
				//dirOffset = findNearestCoterminalAngle(pry, Math.PI/4);
			} else if (input.action.moveBackward && input.action.moveLeft) {
				wasKeyPressed = true;
				dirOffset = 4 * Math.PI / 3;
				//dirOffset = findNearestCoterminalAngle(pry, 4*Math.PI/3);
			} else if (input.action.moveBackward && input.action.moveRight) {
				wasKeyPressed = true;
				dirOffset = 7 * Math.PI / 4;
				//dirOffset = findNearestCoterminalAngle(pry, 7*Math.PI/4);
			} else */if (input.action.moveForward) {
				wasKeyPressed = true;
				//dirOffset = Math.PI / 2;
				//dirOffset = findNearestCoterminalAngle(pry, Math.PI/2);
			} else if (input.action.moveBackward) {
				wasKeyPressed = true;
				//dirOffset = 3 * Math.PI / 2;
				//dirOffset = findNearestCoterminalAngle(pry, 3*Math.PI/2);
			} else if (input.action.moveLeft) {
				wasKeyPressed = true;
				//dirOffset = Math.PI;
				//dirOffset = findNearestCoterminalAngle(pry, Math.PI);
			} else if (input.action.moveRight) {
				wasKeyPressed = true;
				//dirOffset = 0;
				//dirOffset = findNearestCoterminalAngle(pry, 0);
			}
			...
			world1.t.HUDRaycaster.setFromCamera( input.mouse.HUDRay, world1.t.HUDCamera );	
			// calculate objects intersecting the picking ray
			var intersects = world1.t.HUDRaycaster.intersectObjects( world1.t.HUD.children );
			...
			world.t.renderer.render(world.t.HUD, world.t.HUDCamera);
		}
	}
