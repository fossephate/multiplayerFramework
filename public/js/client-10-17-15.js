  ...
  this.createCanvas = function(location, id, width, height) {
    ...
    document.body.appendChild( this.t.renderer.domElement );
		...
		this.c.pw = new CANNON.World();
			this.c.objects = [];
			this.c.pw.gravity.set(0,-9.82,0);
...
//function makeVehicle() {
	...
	var chassisBody = new CANNON.Body({mass:100.0});
	chassisBody.addShape(chassisShape);
	chassisBody.position.set(10, 10, 0);
  ...
  if(world.game.connected) {
    ...
    if(input.key.left) {
      ...
      //cube.rotateOnAxis(new THREE.Vector3(0,1,0), 0.1);
			...
			vehicle.setSteeringValue(rcurrent, 2);
			vehicle.setSteeringValue(rcurrent, 3);
    }
    if(input.key.right) {
      //cube.rotation.y -= 0.1;
      //cube.rotateOnAxis(new THREE.Vector3(0,1,0), -0.1);
